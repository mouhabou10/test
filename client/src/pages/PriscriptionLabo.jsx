...
const PriscriptionLabo = () => {
  ...
  useEffect(() => {
    if (files.length > 0 && files.every(file => file.progress === 100)) {
      ...
      localStorage.setItem("uploadedLaboFileInfos", JSON.stringify(fileInfos));
      window.uploadedLaboFiles = files;
    }
  }, [files]);
  ...
  const handleCancel = (fileName) => {
    setFiles((prevFiles) => {
      ...
      if (updatedFiles.length === 0) {
        localStorage.removeItem("uploadedLaboFileInfos");
        if (window.uploadedLaboFiles) {
          delete window.uploadedLaboFiles;
        }
        ...
        localStorage.setItem("uploadedLaboFileInfos", JSON.stringify(fileInfos));
        window.uploadedLaboFiles = updatedFiles;
      }
      return updatedFiles;
    });
  };
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      localStorage.setItem("laboSearchResults", JSON.stringify(res.data.data));
      navigate("/labo/labo-search-results");
    } catch (err) {
  ...
  return (
    <section className="prescription-page">
      <div className="prescription-container">
        <h1 className="main-title">Lab Prescription</h1>
...
export default PriscriptionLabo;
